  ...
  return (
    <div className="w-full h-full">
      <div className="w-full mb-4 flex items-center justify-between gap-4 border-b rounded p-2">
        ...
        <Button onClick={() => setOpenCrtProdDialog(true)}>Add new product</Button>
      </div>
              ...
              <TableBody>
                {filteredProducts.map((product, index) => (
                  <TableRow key={index} className="hover:bg-gray-100">
                    ...
                    <TableCell
                      className={product?.salePrice > 0 ? 'line-through text-muted-foreground' : ''}
                    >
                      ৳ {product?.price}
                    </TableCell>
                    <TableCell>
        ...
        <SheetContent side="right" className="overflow-auto bg-white">
          <SheetHeader>
            <SheetTitle className="text-2xl text-foreground">
              {currentEditedId ? <p>Edit Product</p> : <p>Create New Product</p>}
            </SheetTitle>
          </SheetHeader>
          <ImageUpload
            file={imageFile}
